...
void j1939_tl_init(void)
{
    ...
    memset(&J1939_rx_message, 0, sizeof(J1939_rx_message));
    memset(&J1939_rx_pdu, 0, sizeof(J1939_rx_pdu));
...
void j1939_tl_process(J1939_RX_PDU_T *pdu_ptr)
{
    if ((pdu_ptr->PGN == TP_CM) || (pdu_ptr->PGN == TP_DT))
    {
        ...
        memcpy((void *)J1939_rx_pdu.data, (void *)pdu_ptr->data, NUMBER_PDU_BUFFERS);
    }
    else
    {
...
void j1939_tl_periodic(void)
{
    ...
    while (go_on)
    {
        ...
        case INIT_REASSEMBLE_STRUCTURE:
        {
            ...
            else if (J1939_rx_state_machine.TP == TP_CM_BAM)
            {
                if (J1939_rx_state_machine.byte_count < NUMBER_TRANS_RX_BUFFERS)
                {
                    ...
                    memset((void *)J1939_rx_message.data, 0, NUMBER_TRANS_RX_BUFFERS);
        ...
        case CHECK_TIMER:
        {
            if (J1939_rx_state_machine.timer_counter > (750 / J1939_PERIODIC_TICK)) // 750ms
            {
        ...
        case FILL_USER_MESSAGE:
        {
            ...
            // send message up the stack to the Network Management Layer
            j1939_nml_process(&J1939_rx_message);
