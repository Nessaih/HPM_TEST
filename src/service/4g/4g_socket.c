...
#define SOCKET_SEND_BUFFER_LEN (3 * IF_4G_SOCKET_MAX_DATA_LEN)
#define SOCKET_DATA_MAX_LEN IF_4G_SOCKET_MAX_DATA_LEN
#define SOCKET_DATA_SLICE_MAX_LEN (1024U)
...
uint8 socket_4g_send(uint8 pri, uint8 conn_id, uint8 *data, uint16 len)
{
    ...
    if (0 == len)
    {
        MODULE_LOG_E(TBOX4G, "the len is 0");
        return 1;
    }

    UINT8 slice_num = (SOCKET_DATA_SLICE_MAX_LEN - 1 + len) / SOCKET_DATA_SLICE_MAX_LEN;
    if (FALSE == dataqueue_can_put_data(&socket_4g_mgr.info[index].send_queue, len, slice_num))
    {
        MODULE_LOG_E(TBOX4G, "send queue is full, send data failed.");
        return 1;
    }

    for (UINT8 slice = 0; slice < slice_num; slice++)
    {
        header.magic_no = DATA_QUEUE_MAGICNO;
        header.element_type = conn_id;
        if (slice != (UINT8)(slice_num - 1U))
        {
            header.element_len = SOCKET_DATA_SLICE_MAX_LEN;
        }
        else
        {
            header.element_len = len % SOCKET_DATA_SLICE_MAX_LEN;
        }

        dataqueue_enqueue(&socket_4g_mgr.info[index].send_queue, &header, data + slice * SOCKET_DATA_SLICE_MAX_LEN);
    }
