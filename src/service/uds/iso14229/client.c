...
/**
 * @brief Read DTC Information (0x19)
 *
 * @param client
 * @param subFunction Sub-function (0x01-0x19)
 * @param data Optional data
 * @param size Data size
 * @return UDSErr_t
 * @addtogroup readDTCInformation_0x19
 */
UDSErr_t UDSSendReadDTCInformation(UDSClient_t *client, uint8_t subFunction,
                                   const uint8_t *data, uint16_t size) {
    UDSErr_t err = PreRequestCheck(client);
    if (err) {
        return err;
    ...
    client->send_buf[0] = kSID_READ_DTC_INFORMATION;
    client->send_buf[1] = subFunction;
