...
#define SECTOR_MAGIC_WORD    0x30424446U

#define KV_MAGIC_WORD        0x3030564BU
...
#define FAILED_ADDR          0xFFFFFFFFU
...
static void update_kv_cache(fdb_kvdb_t db, const char *name, size_t name_len, uint32_t addr)
{
    ...
    uint16_t name_crc = (uint16_t)(fdb_calc_crc32(0U, name, name_len) >> 16U), min_activity = 0xFFFFU;

    for (i = 0U; i < FDB_KV_CACHE_TABLE_SIZE; i++) {
        if (addr != FDB_DATA_UNUSED) {
            ...
            if (db->kv_cache_table[i].name_crc == name_crc) {
                ...
                if (db->kv_cache_table[i].active > 0U) {
...
{
    size_t   i;
    uint16_t name_crc = (uint16_t)(fdb_calc_crc32(0U, name, name_len) >> 16U);
    ...
    for (i = 0; i < FDB_KV_CACHE_TABLE_SIZE; i++) {
        if ((db->kv_cache_table[i].addr != FDB_DATA_UNUSED) && (db->kv_cache_table[i].name_crc == name_crc)) {
            ...
            _fdb_flash_read((fdb_db_t)db, db->kv_cache_table[i].addr + KV_HDR_DATA_SIZE, (void *)saved_name, FDB_KV_NAME_MAX);
            if (0 == strncmp(name, saved_name, name_len)) {
                *addr = db->kv_cache_table[i].addr;
                if (db->kv_cache_table[i].active >= (0xFFFFU - FDB_KV_CACHE_TABLE_SIZE)) {
                    db->kv_cache_table[i].active = 0xFFFFU;
                } else {
...
static uint32_t find_next_kv_addr(fdb_kvdb_t db, uint32_t start, uint32_t end)
{
    ...
    if (NULL != sector && start == sector->empty_kv) {
        return FAILED_ADDR;
    }
#endif

    uint32_t search_step = sizeof(buf) - sizeof(uint32_t);
    for (; start < end && start + sizeof(buf) < end; start += search_step) {
        if (_fdb_flash_read((fdb_db_t)db, start, (uint32_t *)buf, sizeof(buf)) != FDB_NO_ERR) {
            return FAILED_ADDR;
        }
        for (i = 0U; i < search_step && start + i < end; i++) {
            magic = 0UL;
#ifndef FDB_BIG_ENDIAN
            magic = (magic << 8) | buf[i + 3U];
            magic = (magic << 8) | buf[i + 2U];
            magic = (magic << 8) | buf[i + 1U];
            magic = (magic << 8) | buf[i + 0U];
#else
            magic = (magic << 8) | buf[i + 0U];
            magic = (magic << 8) | buf[i + 1U];
            magic = (magic << 8) | buf[i + 2U];
...
static uint32_t get_next_kv_addr(fdb_kvdb_t db, kv_sec_info_t sector, fdb_kv_t pre_kv)
{
    ...
    if (pre_kv->addr.start == FAILED_ADDR) {
        ...
        if (pre_kv->addr.start <= sector->addr + db_sec_size(db)) {
            ...
            if (addr == FAILED_ADDR || addr > sector->addr + db_sec_size(db) || pre_kv->len == 0U) {
...
static fdb_err_t read_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    _fdb_flash_read((fdb_db_t)db, kv->addr.start, (void *)&kv_hdr, sizeof(struct kv_hdr_data));
...
static fdb_err_t read_sector_info(fdb_kvdb_t db, uint32_t addr, kv_sec_info_t sector, bool traversal)
{
    ...
    if (sector->magic != SECTOR_MAGIC_WORD || (sec_hdr.combined != SECTOR_NOT_COMBINED && sec_hdr.combined != SECTOR_COMBINED)) {
...
static fdb_err_t format_sector(fdb_kvdb_t db, uint32_t addr, uint32_t combined_value)
{
    ...
    if (result == FDB_NO_ERR) {
      ...
      // other MCU internal flash
        ...
        _fdb_write_status((fdb_db_t)db, addr + SECTOR_STORE_OFFSET, sec_hdr.status_table.store, FDB_SECTOR_STORE_STATUS_NUM, FDB_SECTOR_STORE_EMPTY, true);

        _fdb_write_status((fdb_db_t)db, addr + SECTOR_DIRTY_OFFSET, sec_hdr.status_table.dirty, FDB_SECTOR_DIRTY_STATUS_NUM, FDB_SECTOR_DIRTY_FALSE, true);

        sec_hdr.magic    = SECTOR_MAGIC_WORD;
        sec_hdr.combined = combined_value;
        sec_hdr.reserved = FDB_DATA_UNUSED;
        result           = _fdb_flash_write((fdb_db_t)db, addr + SECTOR_MAGIC_OFFSET, (void *)(&(sec_hdr.magic)), (sizeof(struct sector_hdr_data) - SECTOR_MAGIC_OFFSET), true);
#endif
...
static void
sector_iterator(fdb_kvdb_t db, kv_sec_info_t sector, fdb_sector_store_status_t status, void *arg1, void *arg2, bool (*callback)(kv_sec_info_t sector, void *arg1, void *arg2), bool traversal_kv)
{
    uint32_t sec_addr, traversed_len = 0;
...
static bool alloc_kv_cb(kv_sec_info_t sector, void *arg1, void *arg2)
{
    ...
    if (sector->check_ok && sector->remain > arg->kv_size + FDB_SEC_REMAIN_THRESHOLD &&
        ((sector->status.dirty == FDB_SECTOR_DIRTY_FALSE) || (sector->status.dirty == FDB_SECTOR_DIRTY_TRUE && !arg->db->gc_request))) {
...
#ifdef FDB_KV_USING_CACHE
        update_sector_empty_addr_cache(db, FDB_ALIGN_DOWN(kv_addr, db_sec_size(db)), kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(kv->name_len) + FDB_WG_ALIGN(kv->value_len));
...
#ifdef FDB_KV_USING_CACHE
            if (!is_full) {
                update_sector_empty_addr_cache(db, sector->addr, kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(kv_hdr.name_len) + FDB_WG_ALIGN(kv_hdr.value_len));
            }
...
#pragma GCC diagnostic pop
    ...
    case FDB_KVDB_CTRL_SET_DEFAULT_KV:
        if (arg != NULL) {
            db->default_kvs = *(struct fdb_default_kv *)arg;
        } else {
...
fdb_err_t fdb_kvdb_init(fdb_kvdb_t db, const char *name, const char *path)
{
    ...
    result = _fdb_init_ex((fdb_db_t)db, name, path, FDB_DB_TYPE_KV);
    if (result != FDB_NO_ERR) {
        goto __exit;
    }
...

    {
        ...
        db_oldest_addr(db) = 0;
        sector_iterator(db, &sector, FDB_SECTOR_STORE_UNUSED, &sector_oldest_addr, &last_sector_status, check_oldest_addr_cb, false);
