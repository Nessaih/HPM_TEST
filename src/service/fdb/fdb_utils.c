...
fdb_err_t _fdb_flash_read(fdb_db_t db, uint32_t addr, void *buf, size_t size)
{
    ...
    if(db->file_mode) {
        ...
        const fal_partition_t *part = (const fal_partition_t *)db->user_data;
        if(NULL != part && fal_read(part, addr, (uint8_t *)buf, size) < 0) {
            result = FDB_READ_ERR;
        }
#endif
    }
...
#ifdef FDB_USING_FAL_MODE
        const fal_partition_t *part = (const fal_partition_t *)db->user_data;
        if(NULL != part && fal_erase(part, addr, size) < 0) {
...
#ifdef FDB_USING_FAL_MODE
        const fal_partition_t *part = (const fal_partition_t *)db->user_data;
        if(NULL != part && fal_write(part, addr, (uint8_t *)buf, size) < 0) {
