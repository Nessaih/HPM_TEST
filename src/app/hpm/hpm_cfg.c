...
#define HPM_CFG_RUN_INFO_NAME "HPM_CFG_RUN_INFO"
...
static VOID hpm_cfg_run_info_write(VOID)
{
    ...
    tbox_cfg_setkv(HPM_CFG_RUN_INFO_NAME, (uint8_t *)&hpm_cfg_run_info, sizeof(hpm_cfg_run_info));
}

static VOID hpm_cfg_run_info_read(VOID)
{
    tbox_cfg_getkv(HPM_CFG_RUN_INFO_NAME, (uint8_t *)&hpm_cfg_run_info, sizeof(hpm_cfg_run_info));
...
INT32 hpm_cfg_get_report_intv(UINT32 *data, INT32 len)
{
    ...
    UINT32 report_intv = 0;

    if (NULL == data || len < TBOX_CFG_REPOT_INTV_LEN)
    {
        MODULE_LOG_E(HPM, "hpm cfg get report intv failed", ret);
        return -1;
    }

    ret = tbox_cfg_read(cfg_id, &report_intv);
    if (0 != ret)
    {
        MODULE_LOG_E(HPM, "hpm cfg get report intv failed, ret: %d", ret);
        return -1;
    }
...
INT32 hpm_cfg_get_report_accof_intv(UINT32 *data, INT32 len)
{
    ...
    UINT32 report_accof_intv = 0;

    if (NULL == data || len < TBOX_CFG_REPOT_INTV_LEN)
    {
        MODULE_LOG_E(HPM, "hpm cfg get report accof intv failed", ret);
        return -1;
    }

    ret = tbox_cfg_read(cfg_id, &report_accof_intv);
    if (0 != ret)
    {
        MODULE_LOG_E(HPM, "hpm cfg get report accof intv failed, ret: %d", ret);
        return -1;
    }

    if (report_accof_intv > 0)
    {
        *data = report_accof_intv;
        return 0;
    }
...
INT32 hpm_cfg_get_server_timeout(UINT32 *data, INT32 len)
{
    INT32 ret = 0;
    TBOX_CFG_ID cfg_id;
    TBOX_CFG_ID_GET(HPMSVRINTV, cfg_id);
    UINT32 server_timeout = 0;

    if (NULL == data || len < TBOX_CFG_REPOT_INTV_LEN)
    {
        MODULE_LOG_E(HPM, "hpm cfg get server timeout failed", ret);
        return -1;
    }

    ret = tbox_cfg_read(cfg_id, &server_timeout);
    if (0 != ret)
    {
        MODULE_LOG_E(HPM, "hpm cfg get server timeout failed, ret: %d", ret);
        return -1;
    }
